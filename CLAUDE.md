...
- ✅ **第 5 步 · 日志 `logging`** —— `storage.py` 里 `logger = logging.getLogger(__name__)`,在 load/save 成功后记 `logger.info(...)`、文件损坏时记 `logger.error(...)`,全部用**惰性格式化**(`logger.info("已保存 %d 条任务", len(data))`,非 f-string)。`cli.py` 加了 `@app.callback()`:`verbose: bool = typer.Option(False, "--verbose", "-v", help=...)`,`basicConfig` 门槛按 verbose 选 INFO/WARNING(默认安静)。约定:**库只 getLogger+记录,应用(cli 入口)才 basicConfig**;`typer.echo`(对用户)与 logging(给开发者)并存不互替。(踩过的点:列表长度是 `len(x)` 不是 `x.len()`——C++ 腔;日志用惰性 `%` 而异常消息用 f-string。)
- ✅ **第 6 步 · 测试 `tests/`** —— `pytest` 装为 dev 依赖。`tests/` 与 `src/` 平级(测试不进包)。`test_core.py`:测 add/mark_done/delete 的正常 + 边界(找不到 id 返回 False)。`test_storage.py`:学了 **fixture**——`tmp_path`(一次性临时目录)+ `monkeypatch.setattr(storage, "DATA_FILE", tmp_path / "todos.json")` 把数据文件重定向到沙盒,避免污染真实文件;测了 round-trip(靠 dataclass 自动 `__eq__` 直接 `assert loaded == tasks`)、文件不存在→`[]`、损坏文件→`pytest.raises(ValueError)`。共 6 个测试全绿。(踩过的点:`is` 比身份、`==` 比值,字面量比较一律用 `==`;`tmp_path / "x"` 用 `/` 不是逗号;写入路径要和 DATA_FILE 完全一致否则触发 FileNotFound 而非 ValueError。`list_tasks`/`models` 太朴素不必测。)
- 🔜 **第 7 步 · 打包与交付**(下一步)—— entry point 在第 4 步已配好(`todo = "todo.cli:app"`,`uv run todo` 可用)。本步剩:填 `pyproject.toml` 的 `description`、写 `README.md`、`uv build` 产出 wheel/sdist、验证安装后 `todo` 命令能独立运行。
